...
/* Private variables ---------------------------------------------------------*/
const uint8_t LCD_ROW_16[] =
{ 0x00, 0x40, 0x10, 0x50 };
const uint8_t LCD_ROW_20[] =
{ 0x00, 0x40, 0x14, 0x54 };
...
void LCD_DIO_Init(LCD_DIO_HandleTypeDef *hlcd)
{
	...
	else if (hlcd->Mode == LCD_8_BIT_MODE) /* TODO: test 8-bit interface */
	{
		...
		__lcd_dio_write_command(hlcd,
		LCD_FUNCTION_SET | LCD_OPT_DL | LCD_OPT_N);
	}
 ...
 * @return None 
 */
void LCD_DIO_DefineChar(LCD_DIO_HandleTypeDef *hlcd, uint8_t code,
		uint8_t bitmap[])
{
